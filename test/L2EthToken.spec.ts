...
describe("L2EthToken TEST", () => {
  ...
  it("should mint and change balance and total supply", async () => {
    ...
    const finalSupply = await l2EthToken.totalSupply();
    const balanceOfWallet = await l2EthToken.balanceOf(walletFrom.address);
  ...
  it("should increase totalSupply and balance when minting tokens, also emit Mint", async () => {
    const initialSupply = await l2EthToken.totalSupply();
    const initialBalanceOfWallet = await l2EthToken.balanceOf(walletFrom.address);
    const amountToMint = ethers.utils.parseEther("10.0");
  ...
  it("should tranfer successfully", async () => {
    ...
    const senderBalanceAfterTransfer = await l2EthToken.balanceOf(walletFrom.address);
    const recipientBalanceAfterTransfer = await l2EthToken.balanceOf(walletTo.address);
  ...
  it("should burn msg.value and emit Withdrawal event", async () => {
    ...
    const tx = await l2EthToken
      .connect(walletFrom)
      .withdraw(l1Receiver.address, { value: ethers.utils.parseEther("10.0"), gasLimit: 5000000, gasPrice });
    console.log(tx);
    ...
    expect(await l2EthToken.totalSupply()).to.equal(
      initialSupply.add(amountToMint).sub(ethers.utils.parseEther("10.0"))
    );
